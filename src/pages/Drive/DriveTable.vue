    ...
    <div>
        ...
        >
            <tr class="pas-toi no-hover" style="cursor: default">
                ...
                <th>
                    Name
                    <!--i @click="clique" class="fas fa-caret-up" :class='classR'></i-->
                </th>
                <th>
                    Serial nbr<!--i @click="clique" class="fas fa-caret-up" :class='classR'></i-->
                </th>
...
export default {
    ...
    methods: {
        ...
        wrapName(name) {
            if (name && name.length > 38) return name.substring(0, 38) + '...';
            else return name;
        },
        wrapName2(name) {
            if (name && name.length > 16) return name.substring(0, 16) + '...';
            else return name;
        },
        getDate(date) {
        ...
        async openFolder(file) {
            if (file.type === 'folder' || !file.type) {
                this.loading = true;
                await store
                    .dispatch('retrieveFolder', file.fileId)
